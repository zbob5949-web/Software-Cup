...
database.py
数据库连接、建表、初始化数据
其他模块通过 from database import get_db_connection 调用
"""
import pymysql
import os
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS digital_human_configs (
                    ...
                    voice_style VARCHAR(100) NULL,
                    appearance TEXT NULL,
                    clothing TEXT NULL,
                    cultural_style TEXT NULL,
                    keywords TEXT NULL,
            ...
            """)
        # 兼容旧表：补充可能缺失的列（MySQL 不支持 ADD COLUMN IF NOT EXISTS）
        try:
            with conn.cursor() as cur:
                cur.execute("ALTER TABLE chat_records ADD COLUMN source VARCHAR(50) NULL")
            conn.commit()
        except Exception:
            pass  # 列已存在则忽略
