...
VRM_DIR = BASE_DIR / "digital_human" / "vrm_avatar"
VRM_MODEL_PATH = BASE_DIR / "digital_human" / "灵小游.vrm"
VRM_MOUTH_PLUS_MODEL_PATH = BASE_DIR / "digital_human" / "灵小游_mouth_plus.vrm"
VENDOR_DIR = BASE_DIR / "digital_human" / "vendor"
...
def _normalize_reply_tone(reply: str) -> str:
    ...
    # 太长时优先保留前两句，避免数字人说得像文档
    parts = re.split(r"(?<=[。！？!?])", text)
    merged = "".join(part for part in parts if part).strip()
    if len(merged) > 120 and len(parts) >= 2:
        merged = "".join(parts[:2]).strip()
...
def digital_human_chat(req: DigitalHumanChatRequest):
    ...
    try:
        ...
        if is_weather_query(question):
            ...
            if nav_reply:
                ...
                data["session_id"] = session_id
                return Result(200, "success", data)
            result = chat_service.process_user_chat(cursor, question, session_id, f"dh_{session_id[:10]}")
            reply = result["reply"]
...
def vrm_avatar_page():
    index_path = VRM_DIR / "index.html"
    if not index_path.exists():
        raise HTTPException(status_code=404, detail="3d digital human page not found")
    return FileResponse(index_path)


@router.get("/digital-human-3d/assets/model.vrm", include_in_schema=False)
def vrm_avatar_model():
    model_path = VRM_MOUTH_PLUS_MODEL_PATH if VRM_MOUTH_PLUS_MODEL_PATH.exists() else VRM_MODEL_PATH
...
@router.get("/digital-human-3d/lipsync.js", include_in_schema=False)
def vrm_avatar_lipsync_module():
    """口型同步 ES Module (LipSyncAnalyzer + VrmLipSyncDriver)"""
    js_path = VRM_DIR / "lipsync.js"
    if not js_path.exists():
        raise HTTPException(status_code=404, detail="lipsync module not found")
    # 必须以 application/javascript 返回, 否则浏览器会拒绝当作 module 解析
    return FileResponse(
        js_path,
...
@router.get("/digital-human-3d/vendor/{file_path:path}", include_in_schema=False)
def vrm_avatar_vendor(file_path: str):
    """提供本地 vendor 资源 (three.js / three-vrm 等), 避免依赖外网 CDN。

    资源由 scripts/fetch_vendor.py 一次性下载到 app/digital_human/vendor/。
    """
    ...
    if not target.is_file():
        raise HTTPException(status_code=404, detail="vendor asset not found")
    media = "application/javascript" if target.suffix in {".js", ".mjs"} else None
...
def _build_plan_route_feature(
    ...
    start_lat, start_lng = _scale_point(raw_lat, raw_lng)
    all_waypoints = [{
        "id": 0,
        "name": "当前位置",
        "lat": start_lat,
...
def leaflet_assets(filename: str):
    ...
    file_path = VRM_DIR / "leaflet" / filename
