...
def get_current_user_info(authorization: str = Header(None), db=Depends(get_db)) -> dict:
    ...
    with db.cursor() as cursor:
        cursor.execute("SELECT phone, role, is_active, is_super_admin FROM users WHERE phone = %s", (phone,))
        row = cursor.fetchone()
    if not row:
        raise HTTPException(status_code=401, detail="用户不存在")
    if row.get("is_active") is False:
        raise HTTPException(status_code=403, detail="账号已被停用")
    return {"phone": row["phone"], "role": row.get("role") or "user", "is_super_admin": bool(row.get("is_super_admin"))}
