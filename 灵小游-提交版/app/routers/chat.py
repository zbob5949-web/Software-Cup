...
# ---------- 删除会话 ----------
@router.delete("/api/user/chat/session/{session_id}")
def delete_session(
    session_id: str,
    identity: dict = Depends(get_user_identity),
    db=Depends(get_db)
    ...
    """删除指定会话的所有消息"""
    if identity["type"] == "guest":
        return Result(40301, "游客模式下无法操作，请注册登录后使用")
    if identity["type"] == "anonymous":
        return Result(401, "请先登录")
    ...
    cursor = db.cursor()
    cursor.execute(
        "DELETE FROM chat_records WHERE session_id = %s AND phone = %s",
        (session_id, phone)
    )
    deleted = cursor.rowcount
    db.commit()
    return Result(200, "success", {"deleted": deleted, "session_id": session_id})
