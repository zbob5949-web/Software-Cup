...
def login_user(conn, phone: str, password: str, code: str, login_ip: str | None = None):
    """
    用户登录 (密码登录 or 验证码登录)
    返回 JWT access token + refresh token 和用户信息
    """
    if phone not in ('admin', 'user') and not validate_phone(phone):
        return {"code": 40031, "msg": "手机号格式错误", "data": None}

    try:
        with conn.cursor() as cursor:
            # 先确定用户存在
            cursor.execute("SELECT id, phone, password, role, is_active FROM users WHERE phone = %s", (phone,))
            ...
            if user_row.get("is_active") is False:
                return {"code": 40302, "msg": "账号已被停用", "data": None}
            ...
            # 更新最后登录时间
            cursor.execute("UPDATE users SET last_login = NOW(), last_login_ip = %s WHERE id = %s", (login_ip, user_id))
            conn.commit()

        # 生成 JWT token
        access_token = create_token(phone, role=role)
