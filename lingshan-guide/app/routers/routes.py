...
def _assemble_route(cursor, row):
    route = {
        ...
        "difficulty": row[3], "description": row[4],
        "hours_min": row[6], "hours_max": row[7],
    }
    if row[5]:
        ids = [int(i) for i in row[5].split(",") if i.strip().isdigit()]
        if ids:
            fmt = ",".join(["%s"] * len(ids))
            cursor.execute(
            ...
            route["spots"] = [
                {"id": s[0], "name": s[1], "duration": s[2], "tips": s[3]}
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if hours is not None:
                ...
                route = _assemble_route(cursor, r[:8])
                hint = f"为您推荐【{route['name']}】(适合 {r[6]}-{r[7]} 小时)。"
                if hours < r[6]: hint += " 提示：时间略紧，建议精简部分景点。"
                elif hours > r[7]: hint += " 提示：时间充裕，可在各景点慢慢欣赏。"
